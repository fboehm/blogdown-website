...
```{r}
MphEM <- function(func_name = "R", max_iter = 10000, max_prec = 1 / 1000, eval, X, Y){
  n_size <- length(eval)
  c_size <- nrow(X)
  d_size <- nrow(Y)
  ...
  for (t in 1:max_iter){
    ...
    logl_new <- logl_const + MphCalcLogL(eval, xHiy, D_l, UltVehiY, Qi) - 0.5 * n_size * logdet_Ve
    if (func_name=='R') {
		...
		B <- t(UltVeh) %*% UltVehiB 
		cs_out <- calc_sigma(func_name, eval, D_l, X, OmegaU, OmegaE, UltVeh, Qi)
		cs_out[[1]] -> Sigma_uu
		cs_out[[2]] -> Sigma_ee
...
# {
MphInitial <- function(em_iter, em_prec, nr_iter, nr_prec, eval, X, Y, l_min, l_max, n_region){
  n_size <- length(eval)
  c_size <- nrow(X)
  d_size <- nrow(Y)
...
# 		CalcLmmVgVeBeta (eval, Xt, &Y_row.vector, lambda, vg, ve, beta_temp, se_beta_temp);
    cl_out <- CalcLmmVgVeBeta(eval, Xt, Y_row, lambda, vg, ve, beta_temp, se_beta_temp)
# 
...
```{r}
MphEM(func_name = "R", max_iter = 10, max_prec = 0.001, eval = eval, X = X, Y = Y)
```
